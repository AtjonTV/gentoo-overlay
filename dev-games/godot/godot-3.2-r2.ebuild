...
src_compile() {
	if [[ "$(usex mono)" = "yes" ]]; then
		echo "Running Mono Build"

		# Build without mono-glue
		escons "mono_glue=no" "${MYSCONS[@]}"

		# Generate mono-glue
		GDOT=godot.x11.tools.64

		if [[ "${ARCH}" == "amd64" ]]; then
			if use llvm; then
				GDOT=godot.x11.tools.64.llvm
			fi
		fi

		if [[ "${ARCH}" == "x86" ]]; then
			if use llvm; then
				GDOT=godot.x11.tools.32.llvm
			else
				GDOT=godot.x11.tools.32
			fi
		fi

		if [[ -e "bin/${GDOT}" ]]; then
			"bin/${GDOT}.mono" --generate-mono-glue modules/mono/glue
		fi
	fi

	# Build with mono-glue
	escons "mono_glue=yes" "${MYSCONS[@]}"
}
